...
# HyloClaw Pause Group Script
# Usage: ./scripts/pause.sh --slug customer_slug
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "Pauses a customer group (stops message processing, notifies customer)"
      exit 0
      ;;
...
# Write IPC task
mkdir -p "$IPC_DIR"
TASK_FILE="$IPC_DIR/pause-${SLUG}-$(date +%s).json"
...
log "Pause task written for $SLUG ($JID)"
