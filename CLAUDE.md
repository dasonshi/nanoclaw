...
Companion `hylo-post-deploy-gate.timer` (system unit, every 30 min, installed by the same `install-healthcheck.sh`) is the **host-side gate** for the `hylo-post-deploy-verify` skill. It runs the skill's cheap detection half directly on the host — one GitHub search API call for merged `auto-draft` PRs in `dasonshi/hylo` in the last hour (curl + python3, no LLM) — and only enqueues a one-off `once` agent task (`hylo-verify-<epoch>`, prompt `Run skill hylo-post-deploy-verify.`) into `scheduled_tasks` when a PR actually merged. Dedup by PR number in `/var/lib/nanoclaw-hylo-gate/enqueued_prs`. Reads `HYLO_GH_PAT` from `/etc/nanoclaw/hylo-monitor.env`. Replaced a `*/30 * * * *` cron row that spun a full Codex/Claude agent every 30 min just to find "no new PR" and exit (~48 wasted agent calls/day, 2026-07-04); that `hylo-post-deploy-verify` cron row is now **paused** (detection is the gate's job; the enqueued task still runs the unchanged, idempotent skill). Two stale duplicate cron rows (`task-1781710502793-atxsrj` dup of verify, `task-1781710509331-brxy0b` dup of weekly-trends) were also paused.
...
1. **Codex OAuth (ChatGPT subscription).** Run `codex login` on the host; this creates `~/.codex/auth.json`. The container mount picks it up and calls `https://chatgpt.com/backend-api/codex` via the Responses API. Tokens are refreshed in place by the runner. Default model: `gpt-5.5` (override with `OPENAI_MODEL`). The ChatGPT-account backend rejects other model names (e.g. `gpt-5-codex`, `gpt-4o`) with HTTP 400.
   ...
   **Reasoning effort.** For gpt-5/o-series models the runner attaches `reasoning: { effort: ... }` to the Responses request, defaulting to **`high`** (deepest reasoning / best quality / slower). Tunable via `OPENAI_REASONING_EFFORT` (e.g. `medium`) — the host passes it through in `container-runner.ts` (matches the `^OPENAI_` env allowlist), and the runner falls back to `high` when unset. Verified the ChatGPT-sub backend accepts `effort: high` (smoke test 2026-06-14). Non-reasoning models (`gpt-4o`) never get the param (gated by `isReasoningModel`). The default change requires a container image rebuild (`./container/build.sh`); the entrypoint compiles the runner source at container start.

   **Token lifecycle / proactive refresh.** The access token lives exactly **10 days**; the `refresh_token` rotates on each use and goes stale from **disuse**. The runner only refreshes in the final 5 min before expiry — too late, so the token died and needed a manual `codex login --device-auth` roughly every 10 days (observed 2026-06-02, 2026-06-12). Fixed by `codex-token-refresh.timer` (system unit, daily 04:00 UTC, runs as `nanoclaw`): it refreshes via `https://auth.openai.com/oauth/token` (`grant_type=refresh_token`, `client_id=app_EMoamEEZ73f0CkXaXp7hrann`), keeping the refresh_token rotating so it never dies. Backs up `auth.json`, probes the new token, rolls back + Telegram-alerts `OPS_NOTIFY_JID` on failure (early warning *before* expiry). The healthcheck's 401 probe (Check 2 above) remains the safety net. Install via `sudo bash /opt/nanoclaw/deploy/install-healthcheck.sh`. If it ever still dies (e.g. an OpenAI absolute-lifetime cap), re-auth manually: `sudo -u nanoclaw codex login --device-auth`.
